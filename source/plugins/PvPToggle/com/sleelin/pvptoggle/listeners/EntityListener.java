      ...
      if (!event.isCancelled()) {
         if (event.getEntity() instanceof Player) {
            ...
            if (event instanceof EntityDamageByEntityEvent) {
               ...
               if ((Boolean)plugin.getGlobalSetting("citizens") && CitizensAPI.getNPCRegistry().isNPC(player)) {
                  proceed = false;
               }

               if (proceed) {
                  this.eventMagic(player, damager, event, "EntityDamage", true);
               }
            }
         }
      ...
      if (!event.isCancelled()) {
         ...
         if (potion.getShooter() instanceof Player) {
            ...
            for(LivingEntity entity : event.getAffectedEntities()) {
               if (entity instanceof Player && !((Player)entity).equals(damager)) {
                  this.eventMagic((Player)entity, damager, event, "PotionSplash", firstdamage);
                  firstdamage = false;
               }
            }
         }
